...
import com.yzw.base.model.json.Form;
...
public class Model<M extends com.jfinal.plugin.activerecord.Model<M>> extends com.jfinal.plugin.activerecord.Model<M>
{
	...
	{
		if(value instanceof String&&StringUtils.isEmpty((String)value))  return (M)this;
	...
	public boolean deleteByIdAndPid(Object id)
	{
		...
		List<Model<M>> list = (List<Model<M>>) list("where pid=?", id);
	 ...
	 * @return
	 */
	public boolean falseDetele(String id){
		return Db.update("update "+tableName+" set status=0 where id=?",id)>0;
	}
	public boolean falseDelete(String id,Integer status){
		return Db.update("update "+tableName+" set status=? where id=?",status,id)>0;
	}

	public boolean batchFalseDelete(String ids){
		return Db.update("update "+tableName+" set status=0 where id in (" + ids + ")")>0;
	}

	public boolean batchFalseDelete(String ids,Integer status){
		return Db.update("update "+tableName+" set status=? where id in (" + ids + ")",status)>0;
	}


	public boolean TotalStock(Integer id , Integer is_total){
		return Db.update("update " + tableName + " set is_total =? where id= (" + id + ")", is_total)>0;
	}
    ...
    {
        return Db.update("update "+tableName+" set status=? where id = (" + id + ")",status)>0;
	...
	public List<M> listOrderByInsertDate(){
		return list(" order by insert_date");
	}
	public List<M> listOrderByInsertDate(String where, Object... params){
		return list(where + " order by insert_date", params);
	}
	...
	public boolean saveAndInsertDate(){
		this.setDate("insert_date");
        this.setDate("last_date");
		return this.save();
	}
	public boolean updateAndLastDate(){
        return this.setDate("last_date").update();
	}
    ...
    /**
     * 多表查询总数 -liuyj 2015-07-29
     * @param sqlExceptSelect
     * @return
     */
    public long getCountByMoreFrom(String sqlExceptSelect)
    {
        long totalRow = 0L;
        List result = Db.query("select count(*) " + DbKit.replaceFormatSqlOrderBy(sqlExceptSelect));
