...
public final class Txt
{
	...
	/**
	 * 创建一个新的Txt字符串处理对象. 可以使用该对象处理与正则表达式相关的操作.
	 * 
	 * @param str
	 *            要被处理的字符串.
	 */
	 ...
	 * @return
	 */
	public static String[] splitDef(String str,String sp){
		
		if(Txt.split(str, sp).length==0){
			
			return new String[]{str};
		}
		else return Txt.split(str, sp);
	}
	...
	/**
	 * 将给定的字符串s转化为一个双精度浮点数. 如果s不能转化, 则返回0.0d. 此方法与Javascript脚本的parseFloat执行方式类似.
	...
	/**
	 * 将给定的字符串s转化为一个整数. 如果s不能转化为一个整数,则返回0. 此方法与Javascript脚本的parseInt执行方式类似.
	 ...
	 * 
	 * @param regStr
	 *            要匹配的正则模式. 由于此方法为完全匹配，因此regStr正则模式往往以^开头，以$结尾.
	...
	/**
	 * 将字符串中的s1替换成s2. 与{@link #replace(String, String)}
	 * 方法不同的是，此方法仅限普通字符串替换，而不使用正则表达式。
	 * 
	 * @param s1
	 *            要替换的普通字符串.
	 * @param s2
	...
	public static String getExt(String fileName)
	{
		...
		try
		{
			String[] exts = Txt.split(fileName, ".");
